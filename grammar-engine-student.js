...
function openGrammarExam(lvl, sess) {
  ...
  // إنشاء overlay للامتحان
  const existing = document.getElementById("grammarExamOverlay");
  if (existing) existing.remove();

  const overlay = document.createElement("div");
  overlay.id = "grammarExamOverlay";
  overlay.style.cssText = `
    position:fixed;inset:0;z-index:9999;
    background:rgba(4,6,14,.97);
    display:flex;flex-direction:column;
    align-items:center;justify-content:flex-start;
    overflow-y:auto;padding:32px 16px 60px;
  `;
  ...
  closeBtn.style.cssText = `
    ...
    color:#c5a059;padding:8px 18px;border-radius:8px;cursor:pointer;
  ...
  examContainer.style.cssText = "width:100%;max-width:820px;margin-top:20px;";
  overlay.appendChild(examContainer);
  document.body.appendChild(overlay);

  // تحميل grammar-engine-student.js إذا لم يكن محمّلاً بعد
  const engineLoaded = typeof GrammarStudentExam !== "undefined";

  const launchExam = () => {
    GrammarStudentExam.start(lvl, sess, examContainer, (result) => {
      // الدرجة رجعت — سجّل في الـ ScardLine بعد 3 ثواني
      setTimeout(() => {
        syncGrammarToScardline(result);
      }, 3000);
    });
  };
  ...
  if (engineLoaded) {
    ...
    const script = document.createElement("script");
    script.src = "grammar-engine-student.js";
    script.onload  = launchExam;
    script.onerror = () => {
      examContainer.innerHTML = `<div style="color:#ff4757;text-align:center;padding:40px">
        ⚠️ Could not load Grammar Exam engine.<br>
        <small>Make sure grammar-engine-student.js is in the same folder.</small>
      </div>`;
    };
    document.body.appendChild(script);
  }
...
// ─── بعت الدرجة لـ Scardline بعد ما GrammarStudentExam سجّل في Scard ──
async function syncGrammarToScardline(result) {
  ...
  try {
    ...
    await fetch(url);
    // تحديث عرض الـ progress gallery بالدرجات الجديدة
